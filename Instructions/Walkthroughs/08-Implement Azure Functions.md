...
10. Enter キーを押すと、関数が実行され、すべての呼び出しがトレースされます。トレースを表示するには、ポータル **「HttpTrigger1」 \| 「コード + テスト」** ブレードに戻って **「監視」** をクリックします。実行履歴(Timestamp)をクリックして、「…」→「Application Insightsでのクエリの実行」をクリックします。ここで、Application Insights（アプリ監視ツール）で、関数呼び出しに対してのクエリを設定することができます。
