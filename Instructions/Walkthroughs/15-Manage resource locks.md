...
2. Azure portal で、「≡」>「リソースグループ」>「作成」画面より、**myRGLocks**という名前で新規リソースグループを作成します（サブスクリプションはAzure Pass、リージョンはデフォルト設定）。作成後リソースグループ画面で名前をクリックし**myRGLocks** リソース グループに移動します。
